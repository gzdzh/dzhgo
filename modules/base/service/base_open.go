...
func (s *BaseOpenService) creatAdminEPS(ctx g.Ctx) (adminEPS interface{}, err error) {
	...
	dzhCore.DBM(baseEpsAdmin).Where("1=1").Delete()
	...
	for _, router := range routers {
		...
		dzhCore.DBM(baseEpsAdmin).Insert(&Api{
			Module:  module,
			Method:  method,
			Path:    path,
			Prefix:  prefix,
	...
	// 读取数据库表生成eps
	// var modules []*Module
	items, _ := dzhCore.DBM(baseEpsAdmin).Fields("DISTINCT module,prefix").All()
	for _, item := range items {
		module := item["module"].String()
		prefix := item["prefix"].String()
		apis, _ := dzhCore.DBM(baseEpsAdmin).Where("module=? AND prefix=?", module, prefix).All()
		var apiList []*Api
...
func (s *BaseOpenService) creatAppEPS(ctx g.Ctx) (appEPS interface{}, err error) {
	...
	// 获取所有路由并更新到数据库表 base_eps_admin
	dzhCore.DBM(baseEpsApp).Where("1=1").Delete()
	routers := g.Server().GetRoutes()
	for _, router := range routers {
		if router.Type == ghttp.HandlerTypeMiddleware || router.Type == ghttp.HandlerTypeHook {
			continue
		...
		// 获取最后一个元素为summary
		summary := routeSplite[len(routeSplite)-1]
		dzhCore.DBM(baseEpsApp).Insert(&Api{
			Module:  module,
			Method:  method,
			Path:    path,
			Prefix:  prefix,
	...
	// 读取数据库表生成eps
	// var modules []*Module
	items, _ := dzhCore.DBM(baseEpsApp).Fields("DISTINCT module,prefix").All()
	for _, item := range items {
		module := item["module"].String()
		prefix := item["prefix"].String()
		apis, _ := dzhCore.DBM(baseEpsApp).Where("module=? AND prefix=?", module, prefix).All()
		var apiList []*Api
		for _, api := range apis {
			apiList = append(apiList, &Api{
				Module:  api["module"].String(),
