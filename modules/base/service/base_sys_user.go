...
func (s *BaseSysUserService) ModifyAfter(ctx context.Context, method string, param g.MapStrAny) (err error) {
	if method == "Delete" {
		...
		userIds.RemoveValue(1)
		// 删除用户时删除相关数据
		dzhCore.DBM(model.NewBaseSysUserRole()).WhereIn("userId", userIds.Slice()).Delete()
	}
	return
}
...
func (s *BaseSysUserService) ServiceInfo(ctx g.Ctx, req *dzhCore.InfoReq) (data interface{}, err error) {
	...
	// 获取角色
	roleIds, err := dzhCore.DBM(model.NewBaseSysUserRole()).Where("userId = ?", resultMap["id"]).Fields("roleId").Array()
	if err != nil {
		return nil, err
	}
...
func (s *BaseSysUserService) ServiceUpdate(ctx context.Context, req *dzhCore.UpdateReq) (data interface{}, err error) {
	...
	err = g.DB().Transaction(ctx, func(ctx context.Context, tx gdb.TX) (err error) {
		roleModel := dzhCore.DBM(model.NewBaseSysUserRole()).TX(tx).Where("userId = ?", userId)
...
func (s *BaseSysUserService) Move(ctx g.Ctx) (err error) {
	...
	_, err = dzhCore.DBM(s.Model).Where("`id` IN(?)", userIds).Data(g.Map{"departmentId": departmentId}).Update()
