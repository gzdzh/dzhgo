...
func (s *TaskInfoService) ModifyAfter(ctx g.Ctx, method string, param g.MapStrAny) (err error) {
	...
	if method == "Add" {
		if gconv.Int(param["status"]) == 1 {
			id, err := dzhCore.DBM(s.Model).Where("name = ?", param["name"]).Value("id")
			if err != nil {
				return err
			}
...
func (s *TaskInfoService) Once(ctx g.Ctx, id int64) error {
	record, err := dzhCore.DBM(s.Model).Where("id = ?", id).One()
	if err != nil {
		return err
	}
	if record.IsEmpty() {
...
func (s *TaskInfoService) Log(ctx g.Ctx, param g.MapStrStr) (data interface{}, err error) {
	...
	taskLog := model.NewTaskLog()
	m := dzhCore.DBM(taskLog).LeftJoin("task_info", "task_info.id = task_log.taskId")
...
func (s *TaskInfoService) SetNextRunTime(ctx g.Ctx, cronId string, cron string) error {
	...
	if e == nil {
		_, err := dzhCore.DBM(s.Model).Where("id = ?", cronId).Data("nextRunTime", nextTime).Update()
		if err != nil {
			return err
		}
	} else {
